void serveCruises(Cruise[] cruises) {
    ...
    for (int cruise = 0; cruise < cruises.length; cruise++) {
        ...
        for (int numOfLoader = 0; numOfLoader < cruises[cruise].getNumOfLoadersRequired(); numOfLoader++) {
            ...
            if (i == 0) {
                if ((noOfLoadersAvailable + 1) % 3 == 0) {
                    loaders[noOfLoadersAvailable] = new 
                        RecycledLoader(noOfLoadersAvailable + 1, cruises[cruise]);
                }
                ...
                System.out.println(loaders[noOfLoadersAvailable]);
